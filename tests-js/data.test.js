...
describe('transformRoute', () => {
  it('uses geometry coords when present', () => {
    const r = transformRoute({
      ...
      country: 'de',
      type: 'offroad',
      geometry: { type: 'LineString', coordinates: [[7, 50], [7.1, 50.1], [7.2, 50.2]] },
    })
  ...
  it('computes duration from distance + type', () => {
    expect(transformRoute({ id: 'a', name: 'A', country: 'de', type: 'touring', distance_km: 90 }).duration_h).toBe(2)
    expect(transformRoute({ id: 'b', name: 'B', country: 'de', type: 'offroad', distance_km: 56 }).duration_h).toBe(2)
  })
...
describe('applyFilters', () => {
  ...
  it('passes all routes with all-filters', () => {
    expect(applyFilters(routes, { country: 'all', type: 'all', difficulty: 'all' })).toHaveLength(3)
  })
  ...
  it('filters by type', () => {
    expect(applyFilters(routes, { country: 'all', type: 'offroad', difficulty: 'all' })).toHaveLength(2)
  })
  ...
  it('combines filters', () => {
    expect(applyFilters(routes, { country: 'de', type: 'offroad', difficulty: 'all' })).toHaveLength(1)
  })
})
