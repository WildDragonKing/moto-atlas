...
function MotoAtlasApp() {
  ...
  const visibleRoutes = useM(() => window.applyFilters(window.ROUTES, filters), [filters]);
  const selectedRoute = useM(() => window.ROUTES.find(r => r.id === selectedId), [selectedId]);
  ...
  useE(() => {
    ...
    map.on('load', async () => {
      ...
      map.addSource('routes', { type: 'geojson', data: buildFC(window.ROUTES) });
  ...
  return (
    <div className="atlas-root">
      ...
      <window.Sidebar
        ...
        allRoutesCount={window.ROUTES.length}
